...
import {
  DEFAULT_SWITCHBOARD_LOG_PATH as POC_WORKFLOW_LOG_PATH,
  DEFAULT_SWITCHBOARD_STORE_PATH as POC_WORKFLOW_STORE_PATH
} from "../src/poc/switchboard_workflow.js";
import { DEFAULT_ROUTE_CONTEXT_PATH as POC_ROUTE_CONTEXT_PATH } from "../src/poc/switchboard_route_context.js";
import { DEFAULT_CLAUDE_HOOK_LOG_PATH as POC_HOOK_LOG_PATH } from "../src/poc/claude_hook_bridge.js";
...
test("poc compatibility wrappers keep their legacy src/poc/logs defaults", () => {
  const expectedDir = path.join(process.cwd(), "src", "poc", "logs");

  assert.equal(POC_WORKFLOW_STORE_PATH, path.join(expectedDir, "switchboard-sessions.json"));
  assert.equal(POC_WORKFLOW_LOG_PATH, path.join(expectedDir, "switchboard-turns.ndjson"));
  assert.equal(POC_ROUTE_CONTEXT_PATH, path.join(expectedDir, "switchboard-route-context.json"));
  assert.equal(POC_HOOK_LOG_PATH, path.join(expectedDir, "claude-hook-events.ndjson"));
});
